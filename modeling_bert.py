...
class BertSelfAttention(nn.Module):
    ...
    def transpose_for_scores(self, x):
        ...
        new_x_shape = np.shape(x)[:-1] + (self.num_attention_heads, self.attention_head_size)
    ...
    def forward(
        ...
        new_context_layer_shape = np.shape(context_layer)[:-2] + (self.all_head_size,)
...
class BertPredictionHeadTransform(nn.Module):
    ...
    def forward(self, hidden_states):
        print("I am at line 559 of modeling_bert.py forward function BertPredictionHeadTransform.")
        hidden_states = self.dense(hidden_states)
...
class BertPreTrainingHeads(nn.Module):
    ...
    def forward(self, sequence_output, pooled_output):
        print("I am at line 611 of modeling_bert.py forward function BertPreTrainingHeads.")
        prediction_scores = self.predictions(sequence_output)
        seq_relationship_score = self.seq_relationship(pooled_output)
...
class BertModel(BertPreTrainedModel):
    ...
    def forward(
        ...
        if self.config.is_decoder and encoder_hidden_states is not None:
            ...
            encoder_batch_size, encoder_sequence_length, _ = np.shape(encoder_hidden_states)
...
class BertForQuestionAnswering(BertPreTrainedModel):
    ...
    def forward(
        ...
        if start_positions is not None and end_positions is not None:
            ...
            if len(np.shape(start_positions)) > 1:
                start_positions = start_positions.squeeze(-1)
            # if len(end_positions.size()) > 1:
            if len(np.shape(end_positions)) > 1:
